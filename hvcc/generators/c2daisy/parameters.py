...
def parse_parameters(parameters, components, aliases, object_name):
  ...
  for param_name, param in params_in.items():
    ...
    param_struct = {"hash_enum": params_in_original_names[param_name], 'name': root, 'type': component['component'].upper()}
    ...
    replacements['callback_write_in'].append({"process": process, "bool": mapping["bool"], "hash_enum": params_in_original_names[param_name]})
  ...
  for param_name, param in params_out.items():
    ...
    component = list(filter_match(components, 'name', root))[0]
    param_struct = {'hash_enum': params_out_original_names[param_name], 'index': out_idx, 'name': param_name}
