...
void sendHook(HeavyContextInterface *c, const char *sendName, hv_uint32_t sendHash, const HvMessage *msg) {
  double timestamp = hv_samplesToMilliseconds(c, hv_msg_getTimestamp(msg));

  switch (sendHash) {
    case HV_HASH_NOTEOUT: {
      float note = hv_msg_getFloat(msg, 0);
      float velocity = hv_msg_getFloat(msg, 1);
      float channel = hv_msg_getFloat(msg, 2);
      printf("[@ %.3f] noteout: %.0f %.0f %.0f\n", timestamp, note, velocity, channel);
      break;
    }
    case HV_HASH_CTLOUT: {
      float value = hv_msg_getFloat(msg, 0);
      float cc = hv_msg_getFloat(msg, 1);
      float channel = hv_msg_getFloat(msg, 2);
      printf("[@ %.3f] ctlout: %.0f %.0f %.0f\n", timestamp, value, cc, channel);
      break;
    }
    case HV_HASH_POLYTOUCHOUT: {
      float value = hv_msg_getFloat(msg, 0);
      float note = hv_msg_getFloat(msg, 1);
      float channel = hv_msg_getFloat(msg, 2);
      printf("[@ %.3f] polytouchout: %.0f %.0f %.0f\n", timestamp, value, note, channel);
      break;
    }
    case HV_HASH_PGMOUT: {
      float pgm = hv_msg_getFloat(msg, 0);
      float channel = hv_msg_getFloat(msg, 1);
      printf("[@ %.3f] pgmout: %.0f %.0f\n", timestamp, pgm, channel);
      break;
    }
    case HV_HASH_TOUCHOUT: {
      float value = hv_msg_getFloat(msg, 0);
      float channel = hv_msg_getFloat(msg, 1);
      printf("[@ %.3f] touchout: %.0f %.0f\n", timestamp, value, channel);
      break;
    }
    case HV_HASH_BENDOUT: {
      float value = hv_msg_getFloat(msg, 0);
      float channel = hv_msg_getFloat(msg, 1);
      printf("[@ %.3f] bendout: %.0f %.0f\n", timestamp, value, channel);
      break;
    }
    case HV_HASH_MIDIOUT: {
      float byte = hv_msg_getFloat(msg, 0);
      printf("[@ %.3f] midiout: %.0f\n", timestamp, byte);
      break;
    }
    default:
      break;
