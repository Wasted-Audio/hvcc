...
Each of these are optional and have either a default value or are entirely optional (description and homepage). Midi i/o ports are off by default, but can be set to `1` and they will be enabled.
...
- `enumerators` - Configure a set of parameters that cycle over a list of string values and return an integer in the patch.
- `enable_ui` - Integer value that can generate different plugin UIs.
  - 0 - The same as disabling the UI, creates nothing.
  - 1 - A generic [Dear Imgui](./dpf_imgui.md) UI. Requires `dpf-widgets` on the same level as `dpf`.
  - 2 - A [NanoVG](./dpf_nanovg.md) based UI that emulates [Plugdata](https://plugdata.org/) graphics. Requires `PDVG` on the same level as `dpf`. It also requires the use of `@hv_param` [receive config](../getting-started/patching.md#gui-objects) on the UI objects. An example project using this can be found [here](https://github.com/Wasted-Audio/pdvg-example-plugin/).
