...
`* Heavy does not support the Pd concept of "ports". Channel numbering of the generator is expect to be the typical 16 midi channels starting at 0. For this reason the midi wrapper objects internally [+ 1] and [- 1] since Pure Data starts channel numbering at 1 and this keeps some expected patch compatibility in place.`
